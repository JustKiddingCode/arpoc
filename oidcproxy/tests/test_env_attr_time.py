...
from oidcproxy.plugins.env_attr_time import *
time_object = datetime.datetime.fromisoformat("2000-07-08 01:02:03")

def test_datetime_object():
    diff = oidcproxy.utils.now_object().timestamp() - datetime.datetime.now().timestamp()
    assert diff <= 0.5

@patch('oidcproxy.utils.now_object',return_value=time_object)
def test_time(mock):
