...
class Log(Obligation):
    ...
    def replace_subjectattr(logtext, subject_info):
        ...
        func = lambda x : reduce(lambda d, key: d.get(key, None) if isinstance(d, dict) else None, x.group('subject').split("."), subject_info)
        return re.sub(regex, func, logtext)

    @staticmethod
    def replace_objectattr(logtext, object_info):
        regex = r"object\.(?P<object>[.\w]+)"
        func = lambda x : reduce(lambda d, key: d.get(key, None) if isinstance(d, dict) else None, x.group('object').split("."), object_info)
        return re.sub(regex, func, logtext)

    @staticmethod
    def replace_envattr(logtext, env_info):
        regex = r"environment\.(?P<env>[.\w]+)"
        func = lambda x : reduce(lambda d, key: d.get(key, None) if isinstance(d, dict) else None, x.group('env').split("."), env_info)
        return re.sub(regex, func, logtext)

    @staticmethod
    def replace_accessattr(logtext, access_info):
        regex = r"access\.(?P<access>[.\w]+)"
        func = lambda x : reduce(lambda d, key: d.get(key, None) if isinstance(d, dict) else None, x.group('access').split("."), access_info)
        return re.sub(regex, func, logtext)
    ...
    def run(effect: Optional[Effects], context: Dict, cfg: Dict) -> bool:
        ...
        log_format = "{} subject.email accessed object.url ".format(str(effect))
